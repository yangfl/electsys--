...
chrome.storage.sync.get('old_sdtMain', item => {
  if (item.old_sdtMain) {
    ...
    window.stop()
    $.get(chrome.extension.getURL('/script/panel/sdtMain/sdtMain.html'), data => {
...
chrome.storage.local.get('clear_localStorage', item => {
  if (item.clear_localStorage) {
    chrome.storage.local.remove('clear_localStorage')
    localStorage.clear() }})
